...
RUN git clone https://github.com/pankaj-thapliyal/apprtc.git
...
RUN echo -e "#!/bin/sh\n" > /go/start.sh \
    && echo -e "`pwd`/google-cloud-sdk/bin/dev_appserver.py --enable_host_checking=false `pwd`/apprtc/out/app.yaml &\n" >> /go/start.sh
...
RUN echo -e "$GOPATH/bin/collidermain -port=8089 -tls=true -room-server=https://apprtc.appinvent.in &\n" >> /go/start.sh
